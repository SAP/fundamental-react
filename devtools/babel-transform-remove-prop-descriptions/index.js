  ...
  return {
    visitor: {
      Program(programPath, state) {
        ...
        const globalOptions = {
          visitedKey: `transform-react-remove-prop-descriptions${Date.now()}`,
        ...
        programPath.traverse({
          ObjectProperty: {
            exit(path) {
              ...
              if (node.computed || node.key.name !== 'propDescriptions') {
                return
              }
          ...
          ClassProperty(path) {
            ...
            if (node.key.name === 'propDescriptions') {
          ...
          AssignmentExpression(path) {
            ...
            if (
              node.left.computed ||
              !node.left.property ||
              node.left.property.name !== 'propDescriptions'
            ) {
              return
            }
