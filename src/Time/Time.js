...
class TimeItem extends Component {
  ...
  //TODO:handle hours change when seconds get to 59>00
  /**
  ...
  increaseTimeObj = (name, time, format12Hours) => {
    if (name == "second" && parseInt(time["minute"]) < 60) {
      let newSecond = parseInt(time["minute"]) + 1;
      this
        .props
        .updateTime(newSecond, "minute");
    }
    if ((name == "minute" && format12Hours && parseInt(time["hour"]) < 12) || (name == "minute" && !format12Hours && parseInt(time["hour"]) < 24)) {
      let newMinute = parseInt(time["hour"]) + 1;
      this
        .props
        .updateTime(newMinute, "hour");
    }
  };
  /**
  ...
  decreaseTimeObj = (name, time) => {
    if (name == "second" && parseInt(time["minute"]) > 0) {
      let newSecond = parseInt(time["minute"]) - 1;
      this
        .props
        .updateTime(newSecond, "minute");
  ...
  _onDown = () => {
    ...
    if (name != "meridiem" && value != NaN && value > 0 && value < parseInt(max)) {
      aux = parseInt(value) - 1;
    } else if (name == "meridiem") {
      aux = CLOCK.indexOf(value)
        ? 0
  ...
  onChange = event => {
    ...
    this
      .props
      .updateTime(aux, this.props.name);
  };
...
export class Time extends Component {
  ...
  render() {
    const {showHour, showMinute, showSecond, format12Hours, id} = this.props;
