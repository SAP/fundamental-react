...
export const FormMessage = props => {
    const { msgType, children } = props;
    return <span className={`fd-form__message${msgType ? '  fd-form__message--' + msgType : ''}`}>{children}</span>;
};

FormMessage.propTypes = {
    msgType: PropTypes.oneOf(['', 'error', 'warning', 'help'])
};
...
export const FormTextarea = props => {
    ...
    return (
        <textarea className="fd-form__control" id={id} defaultValue={children} />
    );
};
